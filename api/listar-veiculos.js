...
export default async function handler(req, res) {
  ...
  const endpoint = process.env.HINOVA_VEICULOS_ENDPOINT || process.env.HINOVA_VEICULOS_URL;
  const apiUrl = joinUrl(baseUrl, endpoint);

  if (!apiUrl) {
    return res.status(500).json({ message: 'Configure HINOVA_VEICULOS_URL ou HINOVA_BASE_URL + HINOVA_VEICULOS_ENDPOINT.' });
  }
  ...
  try {
    ...
    if (!documento && !associadoId) {
      return res.status(400).json({ message: 'Informe o CPF/CNPJ ou ID do associado.' });
    }
    ...
    const data = await requestHinova({
      url: apiUrl,
      method: process.env.HINOVA_VEICULOS_METHOD || 'GET',
      payload,
    });

    const veiculos = findArray(data, ['veiculos', 'veiculos_associado', 'vehicles', 'data', 'dados']);

    return res.status(200).json({ veiculos, data });
  } catch (error) {
    return res.status(error.status || 500).json({
      message: error.message || 'Erro ao listar veículos na API da Hinova.',
