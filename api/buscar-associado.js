...
export default async function handler(req, res) {
  ...
  const endpoint = process.env.HINOVA_ASSOCIADO_ENDPOINT || process.env.HINOVA_ASSOCIADO_URL;
  const apiUrl = joinUrl(baseUrl, endpoint);

  if (!apiUrl) {
    return res.status(500).json({ message: 'Configure HINOVA_ASSOCIADO_URL ou HINOVA_BASE_URL + HINOVA_ASSOCIADO_ENDPOINT.' });
  }

  try {
    const documento = onlyDigits(req.body?.documento);
    ...
    const data = await requestHinova({
      url: apiUrl,
      method: process.env.HINOVA_ASSOCIADO_METHOD || 'POST',
      payload: { [documentField]: documento },
    });

    const associado = findObject(data, ['associado', 'cliente', 'data', 'dados']);

    if (!associado) {
      return res.status(404).json({ message: 'Cadastro não localizado para o CPF/CNPJ informado.', data });
    }

    return res.status(200).json({ associado, data });
  } catch (error) {
    return res.status(error.status || 500).json({
      message: error.message || 'Erro ao buscar associado na API da Hinova.',
